...
class Task3 extends FunSuite{
  ...
  test("enter again"){
    ...
    val item: Item = new Item("pineapple",1.75)
...
//    assert(cart.size == 2)
  ...
  test("clear then enter"){
    ...
    assert(compareDoubles(cart.head.price, 0.0))
    assert(cart(1).description() == "error")
    assert(compareDoubles(cart(1).price(),0.0))

  }

  test("enter enter num clear"){
    val checkout: SelfCheckout = new SelfCheckout()
    val item: Item = new Item("pineapple",1.75)
    ...
    assert(compareDoubles(cart(2).price(),0.0))

  }

  test("empty cart for next customer"){
    val checkout: SelfCheckout = new SelfCheckout()
    val item: Item = new Item("pineapple",1.75)
